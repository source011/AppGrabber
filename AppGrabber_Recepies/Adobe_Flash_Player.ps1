...
$urlVer = "https://get.adobe.com/se/flashplayer/"
$dataVer = Invoke-WebRequest $urlVer
$recepieVersion = $dataVer.AllElements | Where {$_.TagName -eq "strong"}
$recepieVersion = $recepieVersion[0].innerText -replace ("Version ", "")
...
# Check version
	...
	if($recepieVersion -gt $recepieVersionOld){
		...
		try {
            ...
            $msbuild = "$dlLocation\wget\wget.exe"
            $argumentz = "$url2 -P $dlLocation --no-check-certificate"
            start-process -WindowStyle hidden $msbuild $argumentz -wait
            $msbuild = "$dlLocation\wget\wget.exe"
            $argumentz = "$url3 -P $dlLocation --no-check-certificate"
            start-process -WindowStyle hidden $msbuild $argumentz -wait

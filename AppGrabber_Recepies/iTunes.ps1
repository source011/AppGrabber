...
$downloadSw = "https://www.apple.com" + ((Invoke-WebRequest -Uri $url).Links | Where innerHTML -like $search).href
...
$recepieVersion = $regex.Matches($parseVer) | %{ $_.value }
...
if($recepieVersion -gt $recepieVersionOld){
	...
	try {
        ...
        $argumentz = "$downloadSw -P $dlLocation --no-check-certificate"
